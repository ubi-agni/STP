...
% (2<=n<=7, Normalfall: n=6)
	...
	% (2) Fallunterscheidung in die möglichen Profile DD, TD, DT, TT anhand

	% des übergebenen Bewegungsprofils einer v0->vmax->0 Bewegung. Hier

	% wird der DD Fall betrachtet, ansonsten wird bei (3) fortgefahren. 

	% --> DD-Profil. Bei v0>vmax ist DD und DT möglich, ansonsten nur DD.

	% Es wird dementsprechend entweder 'calc7st_nocruise_dt_or_dd' oder

    % direkt 'calc7st_nocruise_dd' aufgerufen.

	

	% (3) DT, TD oder TT-Profil. Im Falle eines TT-Profils kann aus den beiden
...
if ((abs(v0)>vmax) && (sign(ptarget-p0) == dir))
    ...
    [t_reach_vmax j_reach_vmax] = shrink_t_j(t_reach_vmax, j_reach_vmax)

	if (length(t_reach_vmax) == 2)

		t_reach_vmax = t_reach_vmax(1);

		j_reach_vmax = j_reach_vmax(1);

        t_dec = t_reach_vmax(1);

    elseif (length(t_reach_vmax) == 3)
    ...
    if (~isZero(t_dec) && (t_dec > 0))
        ...
        plotaTracks([0 t_reach_vmax],j_reach_vmax,a0,v0);
		...
		[t_res1 j_res1] = calc7st_nocruise(dp,t,dir,trapezoid_stop,ptarget,jmax,amax,vmax,a1,v1,p1)
...
if ((isZero(t(1)-t(2))) && (isZero(t(4)-t(5))))
	...
	if (abs(v0)>vmax)
        ...
        disp(sprintf('Decided on [D-D] in clac7st_nocruise! (2)'));
	...
	if ((dt <= t(2)-t(1)) && (dt <= t(5)-t(4)))
		...
		disp(sprintf('Decided on [T-T] in calc7st_nocruise! (3)'));
	...
	elseif (t(2)-t(1) < t(5)-t(4))
		...
		if (dt <= t(5)-t(4))
            ...
            disp(sprintf('Decided on [D-T] in calc7st_nocruise! (4)'));
            ...
            disp(sprintf('Decided on [D-T]/[D-D] in calc7st_nocruise! (5)'));
        ...
        if (dt <= t(2)-t(1))
            ...
            disp(sprintf('Decided on [T-D] in calc7st_nocruise! (6)'));
            ...
            if ((sign(ptarget-p0) ~= dir) && trapezoid_stop)
			    ...
			    disp(sprintf('Decided on [T-D] in calc7st_nocruise! (7)'));

   				[t_res,j_res] = calc7st_nocruise_td(dir, ptarget,jmax,amax,vmax,a0,v0,p0);

            else

    			disp(sprintf('Decided on [T-D]/[D-D] in calc7st_nocruise! (8)'));
